...
def route_intent(user_input, user_name):
    ...
    # 更新检测
    if _re.search(r'(改到|推迟|提前|延期|延后|推到|推后|往后推|改成|标记为|改成.*完成)', ui):
    ...
    if any(kw in ui for kw in ('吗', '呢', '什么', '哪些', '怎么', '如何', '谁', '几个', '多少')):
        return 'query', {'question': ui}
    # ★ 查询/展示类关键词（"显示所有未完成"/"查看已完成"等）
    if any(kw in ui for kw in ('显示', '查看', '列出', '列表', '看看', '查查', '查一下',
                                 '有什么任务', '有哪些任务')):
        return 'query', {'question': ui}
    if re.search(r'(所有.*任务|全部.*任务|每个人.*任务|大家.*任务)', ui):
        return 'query', {'question': ui}
...
def _detect_batch_complete(user_text, user_name, context, team_name='默认小组', pending_op_id=''):
    ...
    # ====== 通用信号检测 ======
    # ★ 排除查询/展示类输入（"显示所有未完成" 不是在表达批量完成）
    if re.search(r'(显示|查看|列出|列表|看看|查查|有哪些|有什么|查一下)', ui):
        return None
    ...
    # ★ 注意：不用单字"全""都"（会误匹配"安全"中的"全"、"首都"中的"都"）
    has_all_signal = bool(re.search(r'(所有|全部|全都|以上|这些|那些|统统|每个人|大家)', ui))
...
def handle_complete(keyword, user_name, team_name='默认小组'):
    ...
    if not matches:
        return {'result': f'没找到 {user_name} 正在做的「{keyword}」，要新建一条已完成记录吗？', 'intent': 'complete'}, True
    if len(matches) == 1:
        t = matches[0]
        db.supabase.table('records').update({'status': '已完成'}).eq('id', t['id']).execute()
...
def _extract_completer_name(text, current_user):
    ...
    # ★ 优先检测 "XX的" 所有格模式（"王五的安全巡检报告做好了" → 王五）
    # 在 before 中从左到右找第一个 "XX的"（非贪婪，优先匹配最短的）
    possessive = re.search(r'(?:^|(?<=[，,。；;、\s]))(\S{1,5})的(?!\d)', before)
    if possessive:
        pname = possessive.group(1)
        if pname not in ('我', '自己', '你', '他', '她', '它', '谁', '什么'):
            if not re.search(r'(要|把|将|被|给|用|从|对|在|和|与|跟|同)', pname):
                return pname
        if pname == '我':
            return current_user
...
"""
    ...
    # - "我"/"我的" → 只显示当前用户的任务
    # - "张三的"/明确人名 → 显示那人的任务
    # - "所有"/"全部"/"显示所有"/"每个人"/"大家" → 全局查询，不按人过滤
    # - 管理员 → 全局查询时显示全部任务
    user_is_admin = (db.get_user(current_user) or {}).get('is_admin', False) if current_user else False

    # ★ 判断是否为全局查询（显示所有/全部/每个人/列出/查看所有 等）
    is_global_query = bool(
        question and (
            re.search(r'(所有|全部|每个人|大家|每[个位人]|显示|查看|列出|列表)', question) or
            question.startswith('有什么') or question.startswith('有哪些')
        )
    ) if question else False

    # ★ 判断问题是否明确提到"我"（只查自己的任务）
    asks_about_me = bool(
        question and re.search(r'(我[的]|[我我]还|[我我]的|我还|我的)', question)
    ) if question else False

    # ★ 判断问题是否提到其他已知用户名
    target_user = None  # None = 不按人过滤（显示所有）
    ...
    # ★ 可见性过滤：仅当 target_user 明确时才按人过滤
    #   - 全局查询（is_global_query）→ 不按人过滤，管理员看全部，非管理员看公开+自己的
    #   - 按人查询（target_user 不为 None）→ 只看那个人 + 公开任务
    if rows and not is_global_query and target_user is not None:
        ...
        restrict_to_public = (target_user != current_user) and not user_is_admin
            ...
            for r in rows:
                ...
                if task_name == target_user:
                    ...
                    if target_user in [x.strip() for x in assigned.split(',')]:
                        filtered.append(r)
            rows = filtered
    elif rows and is_global_query and not user_is_admin:
        # ★ 非管理员全局查询：只显示自己的 + 公开的 + 分配到的 + 全体
        name_idx = next((i for i, c in enumerate(col_names) if c.lower() == 'name'), None)
        at_idx = next((i for i, c in enumerate(col_names) if c.lower() == 'assigned_to'), None)
        pub_idx = next((i for i, c in enumerate(col_names) if c.lower() == 'is_public'), None)
        if name_idx is not None and current_user:
            filtered = []
            for r in rows:
                task_name = str(r[name_idx] or '')
                is_public = r[pub_idx] in (1, True, '1') if pub_idx is not None else True
                if task_name == current_user:
                    filtered.append(r)
                elif task_name in ('', '全体', '所有人'):
                    filtered.append(r)
                elif is_public:
                    ...
                    assigned = str(r[at_idx] or '')
                    if current_user in [x.strip() for x in assigned.split(',')]:
                        filtered.append(r)
            rows = filtered
...
def _resolve_target_names(request_data, extracted_name, current_user):
    """
    ★ 前端指定人员 vs 文本提取人员 → 智能合并或冲突检测。

    四种结果（返回 (names_list, action)）：
    - action='use_ui'      : 文本没提人名，用 UI 选择的（无冲突）
    - action='use_text'    : UI 没选人，用文本提取的（无冲突）
    - action='conflict'    : UI 选的人 ≠ 文本提取的人，需询问用户
    - action='ambiguous'   : 两边都没指定 → 需询问用户"这是谁的任务？"

    names_list 始终是候选名单。
    """
    assigned_list = request_data.get('assigned_to', [])
    ui_names = [n for n in (assigned_list or []) if n and str(n).strip()]

    # 过滤掉"只有当前用户自己"的情况（视为 UI 未指定）
    if len(ui_names) == 1 and ui_names[0] == current_user:
        ui_names = []

    # 文本提取的名字是否有效（不是当前用户自己、不是"未知"、是真实人名）
    text_has_name = (extracted_name and extracted_name != current_user
                     and extracted_name not in ('未知', ''))

    # ---- 情况1：两边都没指定 → ★ 模糊，反问用户 ----
    if not ui_names and not text_has_name:
        return [current_user], 'ambiguous'

    # ---- 情况2：只有 UI 指定了 → 用 UI ----
    if ui_names and not text_has_name:
        return ui_names, 'use_ui'

    # ---- 情况3：只有文本指定了 → 用文本 ----
    if text_has_name and not ui_names:
        return [extracted_name], 'use_text'

    # ---- 情况4：两边都指定了 → 比较 ----
    # 如果文本提取的人名在 UI 列表中（是子集关系） → 用 UI（UI 可能选了更多人）
    if extracted_name in ui_names:
        return ui_names, 'use_ui'

    # UI 选的人和文本提取的人不一样 → ★ 冲突，需要询问用户
    return list(dict.fromkeys([extracted_name] + ui_names)), 'conflict'
...
def chat():
    ...
    # ---- 处理 人员冲突 选择（complete_name_conflict / add_name_conflict） ----
    if dedup_op_id and dedup_op_id in pending_ops:
        op = pending_ops[dedup_op_id]
        if op.get('type') in ('complete_name_conflict', 'add_name_conflict'):
            user_choice = user_text.strip()
            ui_names = op.get('ui_names', [])
            text_name = op.get('text_name', '')
            all_names = list(dict.fromkeys(ui_names + [text_name]))

            # ★ 鲁棒的序号/选项解析（优先级从高到低）
            choice = None

            # 1) 明确的序号表达："第1个"/"选2"/"我选第一个"/"选项三"/"3"
            ordinal_map = {
                1: [r'(?:^|\s)1(?:\s|$|\.|、|，|。|！|,)', r'第\s*1\s*[个条项]', r'第\s*一\s*[个条项]',
                    r'选项\s*[一1]', r'[选要]\s*[一1](?!\d)', r'^1$', r'^一$',
                    r'前者', r'第一个', r'第一项', r'第1项'],
                2: [r'(?:^|\s)2(?:\s|$|\.|、|，|。|！|,)', r'第\s*2\s*[个条项]', r'第\s*二\s*[个条项]',
                    r'选项\s*[二2]', r'[选要]\s*[二2](?!\d)', r'^2$', r'^二$',
                    r'后者', r'第二个', r'第二项', r'第2项'],
                3: [r'(?:^|\s)3(?:\s|$|\.|、|，|。|！|,)', r'第\s*3\s*[个条项]', r'第\s*三\s*[个条项]',
                    r'选项\s*[三3]', r'[选要]\s*[三3](?!\d)', r'^3$', r'^三$',
                    r'所有', r'全部', r'都[要行可以]', r'大家', r'每个人', r'第三个', r'第三项', r'第3项'],
            }
            for num in [3, 2, 1]:  # 先检查 3 再 2 再 1（"都"不应该被"1"的宽松匹配吃掉）
                for pat in ordinal_map[num]:
                    if re.search(pat, user_choice):
                        choice = num
                        break
            ...
            if choice is None:
                for i, n in enumerate(all_names):
                    if n and n in user_choice:
                        choice = i + 1
                        break

            # 3) 无法识别 → 再次提示
            if choice is None:
                ui_str = '、'.join(ui_names)
                all_str = '、'.join(all_names)
                return jsonify({'result': f'没太明白你的选择，请回复：\n1. {ui_str}\n2. {text_name}\n3. 所有人\n\n（可以说"第一个"、"选2"、"3"、或直接说人名）'})

            # 确定最终人员名单
            if choice == 1:
                final_names = ui_names
            elif choice == 2:
                final_names = [text_name]
            else:  # choice == 3
                final_names = all_names

            pending_ops.pop(dedup_op_id)

            if op.get('type') == 'complete_name_conflict':
                # 为选定人员逐个完成
                kw = op.get('keyword', '')
                tn = op.get('team_name', '默认小组')
                results = []
                for person in final_names[:5]:
                    r, _ = handle_complete(kw, person, tn)
                    results.append(r.get('result', ''))
                return jsonify({'intent': 'complete', 'result': '\n'.join(results)})

            else:  # add_name_conflict
                # 为选定人员逐个创建任务
                action = op.get('action', user_text)
                status = op.get('status', '未完成')
                deadline = op.get('deadline')
                msgs = []
                for person_name in final_names[:5]:
                    r = do_add(op.get('user_text', user_text), person_name, action,
                               status, deadline, team_name=op.get('team_name', '默认小组'),
                               created_by=op.get('current_user', ''))
                    msgs.append(r.get('result', ''))
                return jsonify({'intent': 'add', 'result': '\n'.join(msgs),
                                'multi_person': len(final_names) > 1,
                                'names': final_names[:5]})
    ...
    if intent == 'complete':
        ...
        extracted_un = intent_data.get('user_name', data.get('user', ''))
        current_user = data.get('user', '')

        # ★ 检测 UI 指定人员 vs 文本提取人员 是否冲突
        target_names, resolve_action = _resolve_target_names(data, extracted_un, current_user)

        if resolve_action == 'conflict':
            ui_names = data.get('assigned_to', [])
            ui_str = '、'.join(ui_names)
            text_str = extracted_un
            op_id = str(uuid.uuid4())[:8]
            pending_ops[op_id] = {
                'type': 'complete_name_conflict',
                'keyword': kw, 'team_name': tn,
                'ui_names': ui_names, 'text_name': text_str,
                'current_user': current_user
            }
            return jsonify({
                'intent': 'complete',
                'result': f'⚠️ 你选择了「{ui_str}」，但内容提到的是「{text_str}」。\n\n你是想标记谁的任务为完成？\n1. {ui_str}\n2. {text_str}\n3. 所有人',
                'pending_op_id': op_id,
                'needs_confirmation': True
            })

        if resolve_action == 'ambiguous':
            # ★ 两边都没指定人 → 反问用户
            return jsonify({
                'intent': 'complete',
                'result': f'收到～你说「{kw}」完成了，请问这是谁的任务呢？告诉我姓名就好。',
                'needs_confirmation': True
            })

        # 无冲突 → 使用解析后的人员
        # 如果 UI 指定了多人，为每人尝试完成
        if resolve_action == 'use_ui' and len(target_names) > 1:
            results = []
            for person in target_names[:5]:
                r, _ = handle_complete(kw, person, tn)
                results.append(r.get('result', ''))
            return jsonify({'intent': 'complete', 'result': '\n'.join(results)})
    ...
    if intent == 'confirm':
        ...
        # ★ 检测 UI 指定人员 vs 文本提取人员 是否冲突
        target_names, resolve_action = _resolve_target_names(data, name, current_user)
        ...
        if resolve_action == 'conflict':
            ...
            op_id = str(uuid.uuid4())[:8]
            pending_ops[op_id] = {
                'type': 'add_name_conflict',
                'ui_names': ui_names, 'text_name': text_str,
                'user_text': user_text, 'action': intent_data.get('action', user_text),
                'status': intent_data.get('status', '未完成'),
                'deadline': intent_data.get('deadline'),
                'current_user': current_user, 'team_name': team_name,
        ...
        if resolve_action == 'ambiguous':
            # ★ 两边都没指定人 → 反问用户
            action_desc = intent_data.get('action', user_text)[:30]
            return jsonify({
                'intent': 'add',
                'result': f'收到～你想安排「{action_desc}」，请问这是给谁的任务呢？告诉我姓名就好。',
                'needs_confirmation': True
            })

        # ★ 无冲突 → 使用解析后的人员（可能多人）
        # 如果 UI 指定了多人，为每人各创建一条
        if resolve_action == 'use_ui' and len(target_names) > 1:
            # 多人模式：为每个指定人员各创建一条任务
            ...
            deadline = intent_data.get('deadline')
            msgs = []
            for person_name in target_names[:5]:
                r = do_add(user_text, person_name, action, status, deadline,
                           team_name=team_name, created_by=data.get('user',''))
                msgs.append(r.get('result', ''))
            result = {'intent': 'add', 'result': '\n'.join(msgs),
                      'multi_person': True, 'names': target_names[:5]}
            if isinstance(result, dict) and 'error' in result:
                return jsonify({'result': '❌ ' + result['error'], 'intent': 'add'})
            # 多任务警告前缀
            if multitask_warning:
                if isinstance(result, dict) and 'result' in result:
                    result['result'] = multitask_warning + result['result']
            return jsonify(result)
